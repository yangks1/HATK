...
def extendPattern(ListsTemp, minau):
    ...
    oldPatternDeletePosition = patternInfor[3]

    # 序列扩展
    for item in allItemList:
        index = allItemList.index(item)
        ...
        candidatePatternInforList.append([newPattern, index + 1, newPatternPosition, newPatternDeletePosition])


    # 项集扩展
    for item in allItemList[patternInfor[1]:]:
        index = allItemList.index(item)
        # 得到newPattern
...
def HATKMAIN():
    # 保存高平均效用模式
    # 初始化变量
    global candidatePatternInforList, allItemList
    ListsTemp = [[], []]
    minau = 0
    # 生成1-长度的序列
    for item in utilityTable.keys():
        ...
        candidatePatternInforList.append([[[item]], index + 1, position, deletePosition])
        # candidatePatternNum += 1
    # 模式增长，
