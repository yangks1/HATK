...
'''
更改4
纪录上一个模式的itemSetExitInter集合，只需对增加的项集进行查找是否有相同的项即可
时间优化，占用空间
'''
def utility(pattern):
    """
...
def extendPattern(ListsTemp, minau):
    ...
    oldItemSetExitInter = patternInfor[5][:]
    # 序列扩展
    for item in allItemName.keys():
        if item not in deletedItemSet:
            ...
            for i in range(0, sizeOfPattern):
                itemSetExitInter.append(oldItemSetExitInter[i][:])
            itemSetExitInter.append([])
            for i in range(0, sizeOfPattern):
                if item in patternInfor[0][i]:
                    itemSetExitInter[i].append(sizeOfPattern)
                    itemSetExitInter[sizeOfPattern].append(i)
            """进行计算新模式的位置信息"""
            ...
            if maub > minau:
                ...
                candidatePatternInforList.append([newPattern, indexOfItem + 1, newPatternPosition, maub, newPatternDeletePosition, itemSetExitInter])
    ...
    for item in list(allItemName.keys())[patternInfor[1]:]:
        if item not in deletedItemSet:
            ...
            for i in range(0, sizeOfPattern):
                itemSetExitInter.append(oldItemSetExitInter[i][:])
            for i in range(0, sizeOfPattern-1):
                if item in patternInfor[0][i] and i not in itemSetExitInter[sizeOfPattern-1]:
                    itemSetExitInter[i].append(sizeOfPattern-1)
                    itemSetExitInter[sizeOfPattern-1].append(i)
            for patternIndex in oldPatternPosition.keys():
